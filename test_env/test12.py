# before30after70로 학습 후, before40after60 이용해
# 추가 재학습없이 모델 평가
...
file_path = "../data/before30after70.csv"
...
# split train and test
X_train_after, X_test_after,y_train_after, y_test_after = model_selection.train_test_split(raw_data[feature],
                                                                   raw_data[label],
                                                                   test_size=0.3,
                                                                   random_state=0, 
                                                                   shuffle=True)
...
# Define train and test labels
y_train_LSTM_after = y_train.values.reshape(-1,1)
...
# Evaluate the model
predicted = model.predict(X_test_after, verbose=1, batch_size=10000)

y_predicted = [1 if each > 0.5 else 0 for each in predicted]

score, test_accuracy = model.evaluate(X_test_after, y_test_after, batch_size=10000)
mlflow.log_metric("SCORE", score)
mlflow.log_metric("TEST_ACCURACY", test_accuracy)
...
print("Test Accuracy: ", test_accuracy)
print(metrics.classification_report(list(y_test_after), y_predicted))

# Plot confusion matrix
conf_matrix = metrics.confusion_matrix(y_test_after, y_predicted)
